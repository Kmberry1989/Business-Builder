...
import { Aperture, Palette, Type, Info, Download, RotateCcw, Link as LinkIcon, Phone, Mail, QrCode, Building, Briefcase, Sparkles, Save, Trash2, List, AlignLeft, AlignCenter, AlignRight, PhoneCall, Smartphone, AtSign, Inbox, Globe, Network, ChevronLeft, ChevronRight, Bold, Italic, Underline, Voicemail, Mailbox, MousePointerClick, AlignHorizontalJustifyStart, UserCircle } from 'lucide-react';
...
const createImageObject = (src, size=50) => ({type: 'image', src, size});
const createLineObject = (length=30, thickness=2, color="#000000", style='solid') => ({type: 'line', length, thickness, color, style});
...
const defaultElementPositions = {
    name: { x: 50, y: 15 }, title: { x: 50, y: 28 }, company: { x: 50, y: 41 },
    phone: { x: 50, y: 54 }, email: { x: 50, y: 67 }, website: { x: 50, y: 80 },
    tagline: { x: 50, y: 93 }, monogram: { x: 50, y: 50 }, qrcode: { x: 85, y: 85 },
...
const defaultPresets = {
    "Rochelle Elaine Berry": {
        cardData: {
            name: createTextObject('Rochelle Elaine Berry', true, false, false, '1.3em'),
            title: createTextObject('Creative Strategist & Digital Storyteller', false, true),
            company: createTextObject(''), 
            phone: createTextObject('574.601.5652'), 
            email: createTextObject('rochelleberry731@gmail.com'), 
            website: createTextObject('www.rochelleberry.com'), 
            tagline: createTextObject('Creativity in motion.'), 
            logoUrl: '', 
            headshotUrl: ''
        },
        cardStyle: { 
  ...
  return ( <div ref={ref}> <div ref={cardRef} className={`card-face shadow-lg p-0 overflow-hidden relative ${getCornerRadiusClass()}`} style={cardDynamicStyle} onClick={handleCardClick}> <div className="absolute inset-0 pointer-events-none" style={finishOverlayStyle()}></div> {style.snapGuides?.v != null && <div className="absolute top-0 bottom-0 bg-cyan-400 w-px" style={{left: `${style.snapGuides.v}%`}}></div>} {style.snapGuides?.h != null && <div className="absolute left-0 right-0 bg-cyan-400 h-px" style={{top: `${style.snapGuides.h}%`}}></div>} 
      {(elementsToRender || []).map((elKey) => (
          <DraggableElement key={elKey} elKey={elKey} data={data} cardStyle={style} setCardStyle={setCardStyle} allPositions={style.elementPositions} setSnapGuides={setSnapGuides} isSelected={selectedElement === elKey} onClick={(e) => {e.stopPropagation(); setSelectedElement(elKey);}}>
              {elKey === 'monogram' ? renderMonogram() : <CardElement type={elKey} value={data[elKey]} cardStyle={style} />}
          </DraggableElement>
      ))}
      {style.qrCode?.enabled && style.qrCode.side === side && data.website?.text && 
        <DraggableElement elKey="qrcode" data={data} cardStyle={style} setCardStyle={setCardStyle} allPositions={style.elementPositions} setSnapGuides={setSnapGuides} isSelected={selectedElement === 'qrcode'} onClick={(e) => {e.stopPropagation(); setSelectedElement('qrcode');}}>
            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=${style.qrCode.size}x${style.qrCode.size}&data=${encodeURIComponent(data.website.text)}`} alt="QR Code" style={{width: style.qrCode.size, height: style.qrCode.size}}/>
        </DraggableElement>
      }
    </div> </div> );
});
...
function App() {
  ...
  const handleLoadPreset = (name) => { const p = presets[name]; if (p) { setCardData(p.cardData); setCardStyle(p.cardStyle); } };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 to-sky-100 py-8 px-4 font-sans">
      ...
      <div className="container mx-auto flex flex-col lg:flex-row gap-8 max-w-7xl">
        <CardInputForm {...{ cardData, setCardData, cardStyle, setCardStyle, presets, onSavePreset: handleSavePreset, onLoadPreset: handleLoadPreset, onDeletePreset: handleDeletePreset }}/>
